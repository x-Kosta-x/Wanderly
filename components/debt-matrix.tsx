  ...
  if (participants.length < 2) {
    return (
      <div className="text-center text-gray-500 py-8">
        Добавьте больше участников для отображения матрицы долгов
      </div>
    )
  }
    ...
    <div className="overflow-x-auto">
                  ...
                  <td key={toParticipant.id} className="p-4 text-center border-b border-gray-100">
                    {debt ? (
                      <button
                        ...
                        title={`${fromParticipant.name} должен ${toParticipant.name} ${debt.amount.toFixed(2)} ${getCurrencySymbol(debt.currency)}`}
                      >
                        <div className="flex flex-col items-center">
                          <span className="text-xs font-bold">Кликните</span>
      ...
      <div className="mt-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
        <p className="text-sm text-blue-700 flex items-center gap-2">
          ...
          Нажмите на красную ячейку, чтобы быстро создать перевод для погашения долга
        </p>
      </div>
    </div>
  )
