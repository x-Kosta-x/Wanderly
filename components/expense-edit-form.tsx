  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (deleteSharesError) {
        ...
        toast.error('Ошибка при обновлении долей')
        return
      }
      ...
      if (sharesError) {
        ...
        toast.error('Ошибка при создании долей')
        return
      }
    ...
    >
      ...
      >
        ...
        <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between rounded-t-xl">
          <h2 className="text-xl font-semibold">Редактировать расход</h2>
          <button
            onClick={onClose}
              ...
              {splitMode === 'equal' ? (
                <div className="bg-blue-50 rounded-lg p-4">
                  <p className="text-sm text-blue-800">
                    <span className="font-medium">По {calculateEqualShare().toFixed(2)} {getCurrencySymbol(currency)}</span> с каждого
                    ({selectedParticipants.size} участник{selectedParticipants.size === 1 ? '' : selectedParticipants.size < 5 ? 'а' : 'ов'})
                  </p>
                </div>
              ) : (
                  ...
                  <div className="bg-gray-50 rounded-lg p-3 mt-3">
                    ...
                    {Math.abs(getCustomSharesTotal() - Number(amount || 0)) >= 0.01 && (
                      <div className="text-xs text-red-600 mt-1">
                        Суммы долей не совпадают с общей суммой расхода
                      </div>
                    )}
                  </div>
                </div>
