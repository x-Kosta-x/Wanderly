    ...
    return (
      ...
      >
        <div className="text-center">
          ...
          >
            Добавьте расходы, чтобы увидеть детализацию
          </motion.p>
        </div>
      </motion.div>
    )
    ...
    >
      {Object.entries(expensesByCurrency).map(([currency, currencyExpenses], currencyIndex) => {
        ...
        const expensesByParticipant = currencyExpenses.reduce((acc, expense) => {
          ...
          const payerName = expense.payer?.name || 'Неизвестный'
          if (!acc[payerId]) {
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-semibold text-gray-800">
                    Детализация в {getCurrencySymbol(currency)}
                  </h3>
                  <p className="text-sm text-gray-500">
                    {participantData.length} участник{participantData.length !== 1 ? (participantData.length < 5 ? 'а' : 'ов') : ''}
                  </p>
                </div>
              </div>
              ...
              {topSpender && (
                <div className="flex items-center gap-2 bg-gradient-to-r from-amber-50 to-orange-50 px-3 py-1 rounded-full border border-amber-200">
                  ...
                  <span className="text-sm font-medium text-amber-700">Лидер: {topSpender.name}</span>
                </div>
              )}
            </div>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-2">
                        <div className="flex items-center justify-between text-xs text-gray-500 mb-1">
                          <span>{participant.percentage.toFixed(1)}% от общих расходов</span>
            ...
            >
              <div className="flex items-center justify-between text-sm">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium text-gray-700">Общая сумма расходов</span>
                </div>
