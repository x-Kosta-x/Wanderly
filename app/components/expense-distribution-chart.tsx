    ...
    return (
      ...
      >
        <div className="text-center">
          ...
          >
            Добавьте первый расход, чтобы увидеть диаграмму
          </motion.p>
        </div>
      </motion.div>
    )
            ...
            if (active && payload && payload.length) {
              ...
              return (
                ...
                >
                  ...
                  <p className="text-sm text-gray-500">{data.percentage}% от общих расходов</p>
                </motion.div>
              )
            }
            return null
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      Общая сумма: {totalAmount.toFixed(2)} {getCurrencySymbol(currency)}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-1 text-green-600 bg-green-50 px-3 py-1 rounded-full">
                  <TrendingUp className="w-4 h-4" />
                  <span className="text-sm font-medium">{chartData.length} участник{chartData.length !== 1 ? (chartData.length < 5 ? 'а' : 'ов') : ''}</span>
                </div>
              </div>
