...
export default function TransferForm({ tripId, participants, onClose, initialData }: TransferFormProps) {
  ...
  const handleSubmit = async (formData: FormData) => {
    ...
    try {
      ...
      if (fromId === toId) {
        toast.error('Отправитель и получатель не могут быть одним человеком')
        return
      }
    ...
    >
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-semibold">Перевод денег</h2>
          <button
            onClick={onClose}
        ...
        <form action={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              name="description"
              className="w-full px-3 py-2 border rounded-lg travel-input"
              placeholder="Например: Возврат за такси"
            />
          </div>
