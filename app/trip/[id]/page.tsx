...
export default function TripPage({ params }: { params: { id: string } }) {
  ...
  const handleAddParticipant = async (formData: FormData) => {
    ...
    const { error } = await supabase
      .from('participants')
      .insert([{ name: name.trim(), tripId: params.id }])
  ...
  const handleArchiveTrip = async () => {
    if (!confirm('Вы уверены, что хотите архивировать эту поездку?')) return
    ...
    if (error) {
      ...
      toast.success('Поездка архивирована')
      router.push('/')
    }
  }

  const handleRestoreTrip = async () => {
    if (!confirm('Вы уверены, что хотите восстановить эту поездку из архива?')) return
    ...
    if (error) {
      ...
      toast.success('Поездка восстановлена из архива')
    }
  }
  ...
  const handleDeleteTransfer = async (transferId: string) => {
    ...
    if (error) {
      ...
      toast.error('Ошибка при удалении перевода')
    } else {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Загружаем данные поездки...</p>
        </div>
      </div>
    )
  }
  ...
  if (!trip) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <p className="text-gray-600 mb-4">Поездка не найдена</p>
    ...
    <div className="min-h-screen">
      ...
      <div className="travel-gradient text-white py-8">
        <div className="max-w-7xl mx-auto px-4">
          ...
          {/* Общий баланс группы */}
          <div className="text-center">
            <h2 className="text-xl font-semibold mb-2">Общий баланс группы:</h2>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        {participants.length === 0 ? (
          <div className="travel-card p-8 text-center mb-8">
            ...
            <h3 className="text-xl font-semibold mb-2">Добавьте участников поездки</h3>
            <p className="text-gray-600 mb-6">Начните с добавления участников, чтобы отслеживать расходы</p>
            <button
                ...
                <div className="travel-card p-6">
                  ...
                  {participants.length === 0 ? (
                    <div className="text-center py-8">
                      ...
                      <p className="text-gray-500">Добавьте участников поездки</p>
                    </div>
                  ) : balances.length === 0 ? (
                    <div className="text-center py-8">
                      <Receipt className="w-12 h-12 text-gray-400 mx-auto mb-3" />
                      <p className="text-gray-500">Добавьте расходы, чтобы увидеть балансы</p>
                    </div>
                  ) : (
                    (() => {
                      ...
                      const participantStatuses = participants.map(participant => {
                        ...
                        if (Math.abs(totalOwes - totalOwed) < 0.01) {
                          ...
                          if (mainCreditor) {
                            description = `Должен(на) ${mainCreditor.toName}`
                          } else {
                            description = 'Должен(на)'
                          }
                ...
                <div className="travel-card p-6">
                  ...
                  {expenses.length === 0 ? (
                    <div className="text-center py-12">
                      ...
                      <h3 className="text-lg font-semibold mb-2">Расходов пока нет</h3>
                      <p className="text-gray-600 mb-6">Добавьте первый расход, чтобы начать отслеживание</p>
                      <button
                        onClick={() => setShowExpenseForm(true)}
